...
####Puntos
```{r}
puntos.PC3.completo<- read.csv("/Users/ninamontoya/Desktop/Analisis.2022/analisisR/analisis.final/PCA robusto/puntos.PC3.completo.csv", check.names= F)
```

####Vectores
```{r}
vectores.PC3.completo <- read.csv("/Users/ninamontoya/Desktop/Analisis.2022/analisisR/analisis.final/PCA robusto/vectores.PC3.completo.csv", check.names= F) %>% rename(OTUID=Species)
```

####Matriz de distancia
```{r, eval=FALSE}
matriz.distancia.PC3.completo <- read.delim("/Users/ninamontoya/Desktop/Analisis.2022/analisisR/analisis.final/PCArobusto/distance-matrix.PC3.completo.tsv", check.names = F, row.names = 1)
```

####Taxonomía
```{r}
taxonomia<- read.delim("/Users/ninamontoya/Desktop/Analisis.2022/taxonomia.corridas123/taxonomy.tsv", check.names = F)
```

####ASV´s vectores
```{r, warning=FALSE, message=FALSE}
asvs_vector<- read.delim("/Users/ninamontoya/Desktop/Analisis.2022/analisisR/analisis.final/asvs.vectores.txt", header = F) %>% separate(V1, c("OTUID", "taxa"))
...
####Metadata
```{r, warning=FALSE, message=FALSE}
metadata.transferencia.completo<-read.csv("/Users/ninamontoya/Desktop/Analisis.2022/analisisR/analisis.final/metadata.transf.2600.matriz.csv", check.names = FALSE)
...
puntos.robusto.PC3$orden<-factor(puntos.robusto.PC3$Seccion2,levels=c("Amnioticfluid",
                                                "Embryonic tract","Membrane","Yolk","Mouth","Cloaca",
                                                "Small intestine","Aseptic ventral skin"))
```
